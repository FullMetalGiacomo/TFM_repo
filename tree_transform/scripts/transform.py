...
from tree_transform.msg import PoseStampedArray # importing custom message PoseStampedArray
...
def transform_req(req): # here we call the handler
    ...
    for i in range(len(apriltag.detections)):
        x_tag = apriltag.detection[i].pose.pose.position.x
        y_tag = apriltag.detection[i].pose.pose.position.y
        z_tag = apriltag.detection[i].pose.pose.position.z
        for j in range(len(grapes_wrt_camera.poses)):
            ...
            dist[j,i]=np.sqrt((x2 - x1)**2 + (y2 - y1)**2 + (z2 - z1)**2) # building distance matrix
    ...
    for k in range(len(idx)):
        for t in range(len(grapes_wrt_camera.poses))
            ...
            grapes_wrt_tag_s.header.frame_id = "tag_"+str(apriltag.detections[k].id[0])# creating the frame ID
            grapes_wrt_tag_s.pose=grapes_wrt_camera.poses[t] # taking each pose
...
def tree_transf_server():
    ...
    s = rospy.Service('tree_transf_srv',  tree_transformService, transform_req) # rospy.Service(name, service_class, handler)
    rospy.spin()
