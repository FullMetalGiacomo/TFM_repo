    ...
    for k in range(len(apriltag.detections)):
        ...
        for t in range(len(grapes_wrt_camera.poses)):
   	    rospy.logwarn(t)
            ...
            listener.waitForTransform(grapes_wrt_camera.header.frame_id,"tag_"+str(apriltag.detections[k].id[0]),rospy.Time.now(),rospy.Duration(1.0))
            ...
            grapes_wrt_tag_s.header.frame_id = grapes_wrt_camera.header.frame_id # creating the frame ID
            grapes_wrt_tag_s.pose=grapes_wrt_camera.poses[t] # taking each pose
            grapes_wrt_tag_s=listener.transformPose("tag_"+str(apriltag.detections[k].id[0]),grapes_wrt_tag_s) # taking each pose
